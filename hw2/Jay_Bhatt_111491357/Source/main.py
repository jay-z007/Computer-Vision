...
def Perspective_warping(img1, img2, img3):
    ...
    # Make Room for the side images
    img1 = cv2.copyMakeBorder(img1, 200, 200, 500, 500, cv2.BORDER_CONSTANT)

    # Calculate the Matrix to Transform img2 to the plane of img1
    (M, pts1, pts2, mask) = getTransform(img2, img1, 'homography')

    # Perform the perspective warping for img2
    ...
    # Calculate the Matrix to Transform img3 to the plane of img1
    (M, pts1, pts2, mask) = getTransform(img3, out1, 'homography')

    # Perform the perspective warping for img3
...
def Bonus_perspective_warping(img1, img2, img3):
    ...
    m[:, img1.shape[1] * 11 /
      20:] = 1  # make the mask, ratio is adjusted for best result

    # Perform Laplacian Pyramid blending using mask upto 4 levels
    lpb = Laplacian_Pyramid_Blending_with_mask(out1, img1, m, 4)

    (M, pts1, pts2, mask) = getTransform(img3, out1, 'homography')
    out2 = cv2.warpPerspective(img3, M, (img1.shape[1], img1.shape[0]))

    m = np.ones_like(lpb, dtype='float32')
    m[:, lpb.shape[1] * 9 /
      20:] = 0  # make the mask, ratio is adjusted for best result

    # Perform Laplacian Pyramid blending using mask upto 4 levels
    lpb = Laplacian_Pyramid_Blending_with_mask(out2, lpb, m, 4)
...
def Cylindrical_warping(img1, img2, img3):
    ...
    # Changing the images from cartesian to cylindrical coordinates
    h, w = img1.shape
    # set the focal length to 420
    f = 420
    K = np.array([[f, 0, w / 2], [0, f, h / 2], [0, 0,
                                                 1]])  # mock calibration matrix
    img1_cyl, mask1 = cylindricalWarpImage(img1, K)

    h, w = img2.shape
    f = 420
    K = np.array([[f, 0, w / 2], [0, f, h / 2], [0, 0,
                                                 1]])  # mock calibration matrix
    img2_cyl, mask2 = cylindricalWarpImage(img2, K)

    h, w = img3.shape
    f = 420
    K = np.array([[f, 0, w / 2], [0, f, h / 2], [0, 0,
                                                 1]])  # mock calibration matrix
    img3_cyl, mask3 = cylindricalWarpImage(img3, K)

    # Making room for side images, also doing the same for the mask
    img1_cyl = cv2.copyMakeBorder(img1_cyl, 50, 50, 300, 300,
                                  cv2.BORDER_CONSTANT)
    mask1 = cv2.copyMakeBorder(mask1, 50, 50, 300, 300, cv2.BORDER_CONSTANT)

    # Calculate the Matrix to Transform img2 to the plane of img1
    (M, pts1, pts2, mask) = getTransform(img2_cyl, img1_cyl)

    # USING AFFINE TRANSFORMATION
    # Perform affine transformation on the image and the mask
    mask2 = cv2.warpAffine(mask2, M[:-1], (img1_cyl.shape[1],
                                           img1_cyl.shape[0]))
    out2 = cv2.warpAffine(img2_cyl, M[:-1], (img1_cyl.shape[1],
                                             img1_cyl.shape[0]))

    # stitch the two images, using the mask to choose the valid pixels
    out2 = out2 + img1_cyl * (1.0 - mask2 / 255.0)

    # Calculate the Matrix to Transform img3 to the plane of img1
    (M, pts1, pts2, mask) = getTransform(img3_cyl, img1_cyl)

    # USING AFFINE TRANSFORMATION
    # Perform affine transformation on the image and the mask
    out3 = cv2.warpAffine(img3_cyl, M[:-1], (img1_cyl.shape[1],
                                             img1_cyl.shape[0]))
    mask3 = cv2.warpAffine(mask3, M[:-1], (img1_cyl.shape[1],
                                           img1_cyl.shape[0]))

    # stitch the third image and the previous resultant image, using the mask to choose the valid pixels
    out3 = out3 + out2 * (1.0 - mask3 / 255.0)
...
def Bonus_cylindrical_warping(img1, img2, img3):
    ...
    m = np.zeros_like(img1_cyl, dtype='float32')
    m[:, img1_cyl.shape[1] * 17 /
      30:] = 1  # make the mask, ratio is adjusted for best result

    # Perform Laplacian Pyramid blending using mask upto 3 levels
    lpb = Laplacian_Pyramid_Blending_with_mask(out2, img1_cyl, m, 3)
    ...
    m = np.ones_like(lpb, dtype='float32')
    m[:, lpb.shape[1] * 13 /
      30:] = 0  # make the mask, ratio is adjusted for best result

    # Perform Laplacian Pyramid blending using mask upto 3 levels
    lpb = Laplacian_Pyramid_Blending_with_mask(out3, lpb, m, 3)
